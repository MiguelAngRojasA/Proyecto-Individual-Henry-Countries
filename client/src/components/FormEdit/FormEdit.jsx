...
export default function Form() {
  ...
  function handleChange(event) {
    ...
    if (property === "name") {
      const selectedActivity = sortedActivities.find(activity => activity.id === value);
      if (selectedActivity) {
        setActivityData({...ActivityData, ...selectedActivity});
  ...
  return (
    <div className={style.container}>
      <form className={style.form} onSubmit={handleSubmit}>
        ...
        <button className={style.buttonForm} onClick={() => handeActivityDelete()} >❌Delete Activity❌</button>
        ...
        {Object.keys(errors).length === 0 ? <button>Actualizar</button> : null}
      </form>
    </div>
  );
}